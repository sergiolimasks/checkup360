// performance.js - Performance & A/B Testing Page
const Performance = {
  period: 30,
  activeTab: 'sources',

  async render(container, subPage) {
    this.activeTab = subPage || 'sources';
    container.innerHTML = `
    ...
    if (this.activeTab === 'sources') this.loadSources();
  ...
  async loadSources() {
    ...
    try {
      ...
      if (!rows.length) {
        el.innerHTML = '<div class="dash-card"><div class="dash-card-body"><p class="loading">Nenhum dado de performance encontrado para o período. Os dados aparecem quando leads chegam com parâmetros UTM.</p></div></div>';
        return;
      }
      // Sort controls
      el.innerHTML = `
        <div class="perf-sort-row">
          ...
          <select id="perf-sort" class="filter-select" onchange="Performance.sortTable()">
            ...
            <option value="conversion_rate">Taxa Conversão</option>
            <option value="qualified_rate">Taxa Qualificação</option>
          </select>
        </div>
        <div class="dash-card">
  ...
  sortTable() {
    ...
    tbody.innerHTML = rows.map(function(r) {
      ...
      var campaignLabel = r.campaign && r.campaign !== '-' ? '<div class="perf-campaign">' + esc(r.campaign) + '</div>' : '';
      var mediumBadge = r.medium && r.medium !== '-' ? '<span class="perf-medium">' + esc(r.medium) + '</span>' : '';
      return '<tr>' +
  ...
  renderInsights(rows) {
    ...
    el.innerHTML = '<div class="dash-grid-3">' +
      '<div class="insight-card insight-green"><div class="insight-icon">🏆</div><div class="insight-text"><strong>Melhor Conversão</strong><br>' + esc(bestConv.source || 'direto') + (bestConv.campaign && bestConv.campaign !== '-' ? ' / ' + esc(bestConv.campaign) : '') + '<br><span class="insight-value">' + (bestConv.conversion_rate || 0).toFixed(1) + '% conv.</span></div></div>' +
      '<div class="insight-card insight-blue"><div class="insight-icon">📊</div><div class="insight-text"><strong>Mais Leads</strong><br>' + esc(bestLeads.source || 'direto') + (bestLeads.campaign && bestLeads.campaign !== '-' ? ' / ' + esc(bestLeads.campaign) : '') + '<br><span class="insight-value">' + bestLeads.leads + ' leads</span></div></div>' +
      '<div class="insight-card insight-purple"><div class="insight-icon">💰</div><div class="insight-text"><strong>Mais Receita</strong><br>' + esc(bestRevenue.source || 'direto') + (bestRevenue.campaign && bestRevenue.campaign !== '-' ? ' / ' + esc(bestRevenue.campaign) : '') + '<br><span class="insight-value">R$ ' + Dashboard.fmt(bestRevenue.revenue || 0) + '</span></div></div>' +
    '</div>';
  },
  ...
  async loadABTests() {
    ...
    try {
      ...
      el.innerHTML = `
        ...
        <div id="ab-list">${tests.length ? tests.map(function(t) { return Performance.renderTestCard(t); }).join('') : '<div class="dash-card"><div class="dash-card-body"><p class="loading">Nenhum teste A/B criado ainda. Crie seu primeiro teste para comparar variantes de landing page, criativos ou fontes de tráfego.</p></div></div>'}</div>
      ...
      el.innerHTML = `
        ...
        <div class="dash-card"><div class="dash-card-body"><p class="loading">Endpoint de testes A/B ainda não disponível. Crie testes quando o backend estiver pronto.</p></div></div>
        <div id="ab-modal" style="display:none"></div>`;
    }
  },
  ...
  renderTestCard(test) {
    ...
    var statusLabels = { active:'Ativo', paused:'Pausado', completed:'Concluído' };
    ...
    return '<div class="dash-card ab-test-card" style="margin-bottom:16px;border-left:3px solid ' + color + '">' +
      '<div class="dash-card-header">' +
        ...
        '<div style="display:flex;align-items:center;gap:8px">' +
          ...
          '<span style="color:#64748b;font-size:12px">Início: ' + started + '</span>' +
    ...
    modal.innerHTML = `
      ...
      <div class="modal-box">
        ...
        <div class="form-group">
          ...
          <input type="text" id="ab-name" placeholder="Ex: Headline Dor vs Aspiração">
        </div>
        <div class="form-group">
          <label>Descrição (opcional)</label>
          <input type="text" id="ab-desc" placeholder="O que está sendo testado">
        </div>
        <div class="form-group">
          <label>Métrica Principal</label>
          <select id="ab-metric">
            <option value="conversion_rate">Taxa de Conversão (Lead→Venda)</option>
            <option value="qualified_rate">Taxa de Qualificação</option>
  ...
  addVariantRow() {
    ...
    var letter = String.fromCharCode(65 + count); // C, D, E...
  ...
  async createTest() {
    ...
    if (!name) { resultEl.innerHTML = '<p style="color:#f87171">Nome é obrigatório</p>'; return; }
