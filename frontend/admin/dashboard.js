...
const Dashboard = {
  ...
  renderMainChart(data) {
    ...
    var points = Array.isArray(data) ? data : (data && data.data) ? data.data : [];
  ...
  renderMainChartFallback(chart) {
    ...
    if (!canvas) return;
    if (this.charts.main) this.charts.main.destroy();
    var points = Array.isArray(chart) ? chart : (chart && chart.data) ? chart.data : [];
  ...
  renderFunnel(data) {
    ...
    var stagesArr = data ? (data.stages || data.funnel || null) : null;
    if (!stagesArr || !stagesArr.length) {
      el.innerHTML = '<p class="loading">Sem dados de funil</p>';
      return;
    }
    var stages = stagesArr;
