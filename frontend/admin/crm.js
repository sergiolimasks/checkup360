...
const CRM = {
  ...
  renderCard(lead, now) {
    ...
    // WA window status — janela só abre quando lead manda msg (inbound), usa window_expires_at do banco
    var waStatus = '';
    var windowExpires = lead.window_expires_at ? new Date(lead.window_expires_at) : null;
    if (windowExpires && windowExpires > now) {
      waStatus = '<span class="crm-wa-dot active" title="Janela WA ativa"></span>';
    } else if (lastMsg) {
      waStatus = '<span class="crm-wa-dot expired" title="Janela WA expirada"></span>';
    }
