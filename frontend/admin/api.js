...
const API_BASE = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname.startsWith('100.'))
  ? 'http://' + window.location.hostname + ':3001/api'
  : 'https://api.checkup360.online/api';
...
const Api = {
  ...
  async request(endpoint, options = {}) {
    ...
    if (res.status === 401) {
      ...
      // If an authenticated request fails, clear token and show login
  ...
  // Landing Pages
  getPageStats(period=30) { return this.request('/analytics/pages?period='+period); },
